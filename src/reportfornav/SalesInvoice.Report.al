...
Report 60005 "CCO Sales Invoice"
{
    Caption = 'Sales Invoice';
    Permissions = TableData "Sales Shipment Buffer" = rimd;
    WordLayout = './Layouts/CCOSalesInvoice.docx';
    DefaultLayout = Word;
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(SalesLineVAT; "Sales Invoice Line"."VAT %")
                        {
                            IncludeCaption = false;
                        }
                        ...
                        column(VATTotalText; VATTotalText)
                        {
                            IncludeCaption = false;
                        }
                        ...
                        column(SerialNoText_SalesInvoiceLine; SerialNoText)
                        {
                            IncludeCaption = false;
                        }
                        ...
                        begin
                            ...
                            // >> CC02
                            SerialNoText := '';
                            TempTrackingSpecification.SetRange("Source Ref. No.", "Sales Invoice Line"."Line No.");
            ...
            begin
                ...
                if UpperCase(CopyStr("No.", 1, 4)) = 'VRKR' then
                    InvName := StrSubstNo(Profo, "Sales Invoice Header"."No.")
                else
                    InvName := StrSubstNo(DUE004, "Sales Invoice Header"."No.");
                ...
                case "Sales Invoice Header"."VAT Bus. Posting Group" of
                    ...
                    'NATO_IN':
                        LiefInnerg_DrittlandText := DUE036;
                    'NATO_EU':
                        LiefInnerg_DrittlandText := DUE036;
                end;
                //DUE Ende
                ...
                // >> CC
                if "Sales Invoice Header"."VAT Bus. Posting Group" = 'EU' then begin
                ...
                ItemTrackingDocMgt.RetrieveDocumentItemTracking(TempTrackingSpecification, "Sales Invoice Header"."No.", Database::"Sales Invoice Header", 0);
            ...
            begin
                ...
                if PrintVereinbarung then begin
                    ...
                    if CertificateofSupply.FindFirst then
    ...
    var
        ...
        UstIDText: Text[30];
        DUE024: label 'Exclusively OUR payment terms and conditions are applicable, all other payment terms will not be accepted.';
        DUE031: label 'VAT Registration No.';
        DUE032: label 'VAT-free intra-community delivery acc. to § 4 Nr. 1b UStG';
        DUE033: label 'VAT-free export of goods acc. to § 4 Nr. 1a UStG';
        ...
        DUE036: label 'VAT-Free according to § 4 Nr. 7a UStG in connection with. Art. 67 Abs. 3 NATO ZAbk';
        DUE037: label 'Reverse Charge';
        DUE020: label 'Our general terms and counditions of sale apply.';
        DUE021: label 'You will find our general terms and conditions following www.dueperthal.com or receive them by e-mail upon request.';
    ...
    begin
        with "Sales Invoice Line" do begin
            ...
            if ReportForNavShowOutput and "Sales Invoice Header"."Zeilenrabatt ausblenden" and ("Sales Invoice Line"."Line Amount" <> 0) and ("Sales Invoice Line".Quantity <> 0) then begin
            ...
            if "Sales Invoice Line".Type = "Sales Invoice Line".Type::" " then
                AusgWährTxt := '';
            if ("Sales Invoice Line"."Unit Price" = 0) or ("Sales Invoice Line"."Unit Price" < 0.001) then
                AusgWährTxt := ''
            else
                AusgWährTxt := WährTxt;
        end;
    ...
    local procedure OnPreSectionSalesInvoiceLine_Footer2(var "Sales Invoice Line": Record "Sales Invoice Line");
    begin
        with "Sales Invoice Line" do begin
            ReportForNavSetShowOutput(GLAccReaded = true);
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" <= 20200630D) then
                RechnungMWST := '19% MwSt.-Betrag:'
            else
                RechnungMWST := RechnungMWstCpt;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20200630D) and ("Sales Invoice Header"."Posting Date" <= 20201231D) then
                RechnungMWST := '16% MwSt.-Betrag:'
            else
                RechnungMWST := RechnungMWstCpt;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20201231D) then
                RechnungMWST := '19% MwSt.-Betrag:'
            else
                RechnungMWST := RechnungMWstCpt;
    ...
    local procedure OnPreSectionSalesInvoiceLine_Footer4(var "Sales Invoice Line": Record "Sales Invoice Line");
    begin
        with "Sales Invoice Line" do begin
            ReportForNavSetShowOutput(GLAccReaded = true);
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" <= 20200630D) then
                VorauscptMWST := '19% MwSt.-Betrag:'
            else
                VorauscptMWST := VorauscptMWstCpt;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20200630D) and ("Sales Invoice Header"."Posting Date" <= 20201231D) then
                VorauscptMWST := '16% MwSt.-Betrag:'
            else
                VorauscptMWST := VorauscptMWstCpt;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20201231D) then
                VorauscptMWST := '19% MwSt.-Betrag:'
    ...
    begin
        with "Sales Invoice Line" do begin
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" <= 20200630D) then
                VATTotalText := '19% MwSt.-Betrag:'
            else
                VATTotalText := VATAmountLine.VATAmountText;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20200630D) and ("Sales Invoice Header"."Posting Date" <= 20201231D) then
                VATTotalText := '16% MwSt.-Betrag:'
            else
                VATTotalText := VATAmountLine.VATAmountText;
            if ("Sales Invoice Header"."Sell-to Country/Region Code" = 'DE') and ("Sales Invoice Header"."Posting Date" > 20201231D) then
                VATTotalText := '19% MwSt.-Betrag:'
    ...
    local procedure OnPreSectionSalesInvoiceLine_Body4(var "Sales Invoice Line": Record "Sales Invoice Line");
    begin
        with "Sales Invoice Line" do begin
            // >> CC02
    ...
    begin
        with Total do begin
            ReportForNavSetShowOutput(("Sales Invoice Header"."Ship-to Country/Region Code" <> 'DE') or ("Sales Invoice Header"."VAT Bus. Posting Group" = 'NATO_IN'));
            case "Sales Invoice Header"."VAT Bus. Posting Group" of
                'AUSL.':
                    Zusatz := DUE033;
                'EU':
        ...
        case (dataItemId) of
            ...
            'SalesInvoiceLine':
                begin
                    ...
                    ReportForNavShowOutput := true;
                    OnPreSectionSalesInvoiceLine_Body4("Sales Invoice Line");
                    ...
                    with "Sales Invoice Line" do begin
                        ...
                        ReportForNavShowOutput := true;
                        OnPreSectionSalesInvoiceLine_Footer2("Sales Invoice Line");
                        jsonObject.Add('$Footer2$Pre', ReportForNavShowOutput);
                        ReportForNavShowOutput := true;
                        OnPreSectionSalesInvoiceLine_Footer4("Sales Invoice Line");
